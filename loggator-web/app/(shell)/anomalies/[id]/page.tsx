...
export default async function AnomalyDetailPage({
  ...
  const sourceInfo = SOURCE_LABELS[anomaly.source ?? "llm"] ?? SOURCE_LABELS.llm;
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className={`px-2 py-0.5 rounded border text-[10px] font-medium ${sourceInfo.cls}`}>
            {sourceInfo.label}
          </span>
          <span
      ...
      {anomaly.mitre_tactics?.length > 0 && (
        <div className="space-y-1.5">
          <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
            MITRE ATT&amp;CK Tactics
          </div>
          <MitreBadge tactics={anomaly.mitre_tactics} linkable size="sm" />
        </div>
      )}
      ...
      <div className="grid grid-cols-[1fr_320px] gap-6">
        ...
        <div className="space-y-6">
          ...
          {anomaly.enrichment_context && (
            <EnrichmentPanel enrichment={anomaly.enrichment_context} />
          )}
        ...
        <div className="space-y-4">
          ...
          {/* Triage panel */}
          <TriagePanel anomalyId={anomaly.id} currentStatus={anomaly.triage_status} currentNote={anomaly.triage_note} />
        </div>
      </div>
    </div>
  );
